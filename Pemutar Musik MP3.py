...
#Functions
def play_song():
    filename = filedialog.askopenfilename(initialdir="C:/",title="Silahkan pilih satu file")
    ...
    try:
        ...
        print(e)
        song_title_label.config(fg="red", text="Kesalahan saat memutar trek")
...
def reduce_volume():
    try:
        ...
        print(e)
        song_title_label.config(fg="red",text="Lagu belum anda pilih")
...
#Main Screen
master = Tk()
master.title("Pemutar Musik MP3")

#Labels
Label(master,text="Music Player",font=("sans",16),fg="green").grid(sticky="N",row=0,padx=200)
Label(master,text="Silahkan pilih trek musik yang ingin anda putar",font=("Calibri",12),fg="blue").grid(sticky="N",row=1)
...
#Buttons
Button(master, text="Pilih lagu", font=("Calibri",12),command=play_song).grid(row=2,sticky="N")
...
Button(master, text="+",font=("Calibri",12),width=5,command=increase_volume).grid(row=5,sticky="W")
Button(master, text="-",font=("Calibri",12),width=5,command=reduce_volume).grid(row=5,sticky="E")
